...
export function extractPackedString(bundleJsText) {
    ...
    let i = eq + 1;
    while (i < bundleJsText.length && /\s/.test(bundleJsText[i] ?? ''))
        i++;
...
export function buildNestedPathIndex(resolutions, projectRoot) {
    const index = new Map();
...
export async function generateAddonsManifest(options) {
        ...
        for (const candidate of candidates) {
            try {
                if (fs.existsSync(candidate)) {
                    pkgJson = JSON.parse(await fsp.readFile(candidate, 'utf8'));
                    break;
                }
            }
            catch (err) {
    ...
    addons.sort();
    const bundleId = typeof header.id === 'string' && header.id.length > 0 ? header.id : 'unknown';
    const manifest = {
        version: 1,
        bundleId,
        addons
