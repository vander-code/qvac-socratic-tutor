...
async function detectSilences(audioPath, duration) {
...
function createAudioSegments(duration, silences) {
    ...
    while (duration - start > MAX_SEGMENT_S) {
        ...
        const candidates = silenceMidpoints.filter((candidate) => candidate >= minimumCut && candidate <= maximumCut);
        const firstCandidate = candidates[0];
        const cut = firstCandidate === undefined
...
async function decodeSegment(audioPath, segment) {
    const result = await runCommand('ffmpeg', [
        '-hide_banner',
        '-loglevel',
        'error',
...
if (!args[0]) {
    console.error('Usage: bun run examples/asr/parakeet-tdt-filesystem-chunked.ts ' +
...
try {
    ...
    console.log(`▸ Audio duration: ${formatTimestamp(duration)}; detected ${silences.length} silence interval(s); planned ${segments.length} segment(s)`);
    ...
    for (const [index, segment] of segments.entries()) {
        console.log(`▸ Segment ${index + 1}/${segments.length}: ${formatTimestamp(segment.start)} - ${formatTimestamp(segment.end)}`);
