...
export function listWindowsAudioDevices() {
    const result = spawnSync('ffmpeg', ['-hide_banner', '-f', 'dshow', '-list_devices', 'true', '-i', 'dummy'], { encoding: 'utf8' });
...
export function getAudioInputArgs() {
    ...
    switch (platform()) {
        ...
        case 'win32': {
            ...
            if (!deviceName) {
                throw new Error('No Windows audio input device found. List devices with:\n' +
                    '  ffmpeg -hide_banner -f dshow -list_devices true -i dummy\n' +
...
export function startMicrophone(options) {
    const formatArgs = options.format === 'f32le'
    ...
    if (!ffmpeg.stdout)
        throw new Error('Failed to open microphone');
    return ffmpeg;
}
