...
exports.normalizeBcp47Lang = normalizeBcp47Lang;
exports.getBergamotFileNames = getBergamotFileNames;
exports.hasBergamotModelFiles = hasBergamotModelFiles;
exports.downloadBergamotFromFirefox = downloadBergamotFromFirefox;
exports.ensureBergamotModelFiles = ensureBergamotModelFiles;
...
const BCP47_LANG_ALIASES = Object.freeze({
    zh: "zh-Hans",
});
...
function getBergamotFileNames(srcLang, dstLang) {
    ...
    const cjk = ["zh", "ja", "ko"];
    const separateVocab = cjk.includes(dstLang) ||
...
function hasBergamotModelFiles(dir) {
    try {
        ...
        return (files.some((f) => f.includes(".intgemm")) &&
...
async function downloadBergamotFromFirefox(srcLang, dstLang, destDir) {
    ...
    const pairRecords = records.filter((r) => r.fromLang === fromLangMatch && r.toLang === toLangMatch && r.attachment);
    if (pairRecords.length === 0) {
        throw new Error(`No Firefox Translations model found for ${srcLang}-${dstLang}. ` +
