 ...
 */
import { readModelExecutionMs } from './model-execution.js';
...
registerOperationMetrics({
    ...
    fromFinalChunk: (res) => {
        if (!res.stats)
            return undefined;
        const gauges = {};
        if (res.stats.modelLoadMs !== undefined)
            gauges['modelLoadMs'] = res.stats.modelLoadMs;
        if (res.stats.stepMs !== undefined)
            gauges['stepMs'] = res.stats.stepMs;
        if (res.stats.totalStepMs !== undefined)
            gauges['totalStepMs'] = res.stats.totalStepMs;
        if (res.stats.totalSteps !== undefined)
            gauges['totalSteps'] = res.stats.totalSteps;
        if (res.stats.totalFrames !== undefined)
            gauges['totalFrames'] = res.stats.totalFrames;
        if (res.stats.frames !== undefined)
            gauges['frames'] = res.stats.frames;
        return Object.keys(gauges).length > 0 ? gauges : undefined;
    }
});
registerOperationMetrics({
    ...
    getTags: (req) => (req.modelId ? { modelId: req.modelId } : {}),
    fromFinalChunk: (res) => {
        if (res.stats?.sceneCreateMs === undefined)
            return undefined;
