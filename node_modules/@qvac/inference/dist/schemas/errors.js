...
const errorDefinitions = {
    ...
    [ERROR_CODES.PLUGINS_NOT_REGISTERED]: {
        ...
        message: () => 'No plugins registered. Assemble the engines you need with `plugins([...])` or `registerPlugin(...)` before your first call — import each from its subpath, e.g. `@qvac/inference/llamacpp-completion/plugin`.'
    },
    // Profiler
    ...
    [ERROR_CODES.UNKNOWN_MODEL_TYPE]: {
        ...
        message: (modelType) => `Unknown model type: ${modelType}. Register the plugin for "${modelType}" in code with \`registerPlugin\` / \`plugins([...])\` before this call.`
    },
    // Model loading
    ...
    [ERROR_CODES.CONTEXT_OVERFLOW]: {
        ...
        message: (promptTokens, ctxSize, modelId, cachedTokens, requiredTokens) => {
    ...
    [ERROR_CODES.INSECURE_MODEL_SOURCE]: {
        ...
        message: (url, reason) => `Refusing insecure model download from ${url}: ${reason}`
    },
    [ERROR_CODES.CHECKSUM_UNAVAILABLE]: {
        name: 'CHECKSUM_UNAVAILABLE',
        message: (url) => `Model download from ${url} could not be verified against a trusted checksum ` +
    ...
    [ERROR_CODES.CONFIG_ALREADY_SET]: {
        ...
        message: 'Config has already been set and is immutable. Config can only be set once during initialization.'
    },
    // Lifecycle
    ...
    [ERROR_CODES.ASYNC_DISPOSE_UNAVAILABLE]: {
        ...
        message: 'Host runtime does not expose Symbol.asyncDispose; request-lifecycle primitives require ES2024 `using`/`asyncDispose` support. Verify your runtime (Bare ≥ 1.24) and any polyfill registration.'
    },
    // RPC transport
