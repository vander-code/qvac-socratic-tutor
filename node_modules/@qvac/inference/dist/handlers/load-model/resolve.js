...
import { hyperdriveUrlSchema, registryUrlSchema, SUPPORTED_ARCHIVE_EXTENSIONS, modelInputToSrcSchema } from '../../schemas/index.js';
import { getModelsCacheDir, getShardedModelCacheDir, generateShortHash, extractAndValidateShardedArchive, measureChecksum } from '../../utils/index.js';
...
import { ChecksumValidationFailedError, DownloadCancelledError, InferenceCancelledError, ModelFileNotFoundError, ModelLoadFailedError, ModelNotFoundError, SeedingNotSupportedError } from '../../errors/index.js';
...
export async function resolveFallbackModel(fallbackSrc, expectedChecksum, progressCallback, mode, signal, hooks, security) {
    ...
    const result = await resolveModelPathInner(fallbackSrc, progressCallback, false, mode, signal, hooks, undefined, security);
...
export async function resolveRegistryModel(params) {
    try {
        ...
        const isCorrectnessError = error instanceof ChecksumValidationFailedError ||
...
async function resolveModelPathInner(modelSrc, progressCallback, seed, mode, signal, hooks, fallbackSrc, security) {
    if (signal?.aborted) {
    ...
    if (fallbackSrc !== undefined && !srcString.startsWith('registry://')) {
        throw new ModelLoadFailedError('fallbackSrc is only supported when modelSrc is a built-in registry model.');
    }
    ...
    if (srcString.startsWith('registry://')) {
        ...
        if (fallbackSrc !== undefined &&
            ...
            throw new ModelLoadFailedError('fallbackSrc is not supported for sharded or multi-file models.');
        }
        return resolveRegistryModel({
            ...
            resolveFallback: () => {
                logger.info(`Registry download failed for ${registryPath}; trying fallbackSrc: ${fallbackSrc}`);
                return resolveFallbackModel(fallbackSrc, expectedChecksum, progressCallback, mode, signal, hooks, security);
            },
            buildUnreachableError: (cause) => new ModelLoadFailedError(`Could not download "${modelMetadata?.modelId}" from the registry on this network ` +
                `(${cause instanceof Error ? cause.message : String(cause)}). ` +
                'If the registry is not reliably reachable here, pass a fallbackSrc ' +
                '(an HTTP URL or local file path) to load this model from an alternate source.', cause)
        });
    }
...
export async function resolveModelPath(modelSrc, progressCallback, seed, signal, hooks, fallbackSrc, security) {
    const result = await resolveModelPathInner(modelSrc, progressCallback, seed, 'base', signal, hooks, fallbackSrc, security);
    return result.path;
}
export async function resolveModelPathWithStats(modelSrc, progressCallback, seed, signal, hooks, fallbackSrc, security) {
    return resolveModelPathInner(modelSrc, progressCallback, seed, 'stats', signal, hooks, fallbackSrc, security);
}
